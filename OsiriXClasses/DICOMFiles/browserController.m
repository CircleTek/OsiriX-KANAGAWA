...
- (BOOL)computeEnoughMemory: (NSArray*)toOpenArray :(unsigned long*)requiredMem
{
	...
	for( int x = 0; x < [toOpenArray count]; x++)
	{
		...
		if( [loadList count])
		{
            ...
            /*
             *below, comment out by kobayashi 2016/4/3
             *
             */
            //else
			//	testPtr[ x] = nil;
			...
			//	NSLog(@"Failed to allocate memory for-TEST1: %llu Mb", (memBlock) / (1024 * 1024));
			//}
		...
		while( enoughMemory == NO)
		{
			...
			for( unsigned long x = 0; x < [toOpenArray count]; x++)
			{
				...
				if( [loadList count])
				{
                    ...
                    testPtr[ x] = malloc( (memBlock * sizeof(float)) + 8192);
					if( testPtr[ x] == nil)
					{
